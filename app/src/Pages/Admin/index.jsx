  ...
  const widgetMenu = (
    <Menu mode="inline" openKeys={openKeys} selectedKeys={keyActive} onOpenChange={onOpenChange}>
      <Menu.SubMenu title="Thống kê" key="chart" icon={<AreaChartOutlined />}>
        <Menu.Item key="chart-user"><NavLink to={`/admin/chart-user`} onClick={onClose}>Người dùng</NavLink></Menu.Item>
        <Menu.Item key="chart-project"><NavLink to={`/admin/chart-project`} onClick={onClose}>Dự án</NavLink></Menu.Item>
      </Menu.SubMenu>
      <Menu.SubMenu title="Tài nguyên" key="resources" icon={<GithubOutlined />}>
        <Menu.Item key="resources-html"><NavLink to={`/admin/resources-html`} onClick={onClose}>Phần tử html</NavLink></Menu.Item>
        <Menu.Item key="resources-css"><NavLink to={`/admin/resources-css`} onClick={onClose}>Class css</NavLink></Menu.Item>
      </Menu.SubMenu>
      <Menu.SubMenu title="Quản lý người dùng" key="users" icon={<UsergroupAddOutlined />}>
        <Menu.Item key="users-list"><NavLink to={`/admin/list-user`} onClick={onClose}>Người dùng</NavLink></Menu.Item>
      </Menu.SubMenu>
    </Menu>
  );
  return <>
    <nav onMouseEnter={showDrawer} className="bg-custom fixed top-0 w-16 h-screen shadow-2xl border-r-2 float-left left-0 top-o ">
      ...
      <ul className="flex flex-col justify-center gap-y-4">
        ...
        <li><GithubOutlined className={openKeys.includes("resources") ? "block text-blue-600" : "block"}/></li>
        <li><UsergroupAddOutlined className={openKeys.includes("list-user") ? "block text-blue-600" : "block"}/></li>
    ...
    <Drawer
      title={<div className="relative"> <img src={logo} alt="logo" className="absolute w-14 h-14 ml-1 -top-3.5"/><h1 className="ml-20">Hệ thống Lgo</h1></div>}
      placement="left"
      closable={false}
      onClose={onClose}
      open={open}
